...
router
  ...
  .post(auth("adminAccess"), OrderControllers.AdminAddOrder);
router
  .route("/get-myorder")
  .get(auth("manageUsers"), OrderControllers.getMyOrders);
router
  .route("/admin-all-orders")
  .get(auth("adminAccess"), OrderControllers.admingetAllOrders);
router
  .route("/get-all-orders-export")
  .get(auth("adminAccess"), OrderControllers.adminGetAllOrdersToExport);
router
  .route("/resendShipping-Confirmation-email/:id")
  .post(auth("adminAccess"), OrderControllers.sendShippingConfirmationMail);

router
  .route("/getOrderbyId/:id")
  .get(auth("adminAccess"), OrderControllers.getOrdersById);
router
  .route("/get-sales-by-brand")
  .get(auth("adminAccess"), OrderControllers.SalesByBrand);
router
  .route("/get-products-by-category/:id")
  .get(auth("adminAccess"), OrderControllers.getProductbycategoryid);

router
  .route("/update-order/:id")
  .put(auth("adminAccess"), OrderControllers.updateOrder);
router
  .route("/update-order-byid/:id")
  .put(auth("adminAccess"), OrderControllers.updateOrderById);
...
router
  .route("/add-trackingNo/:id")
  .put(
    auth("adminAccess"),
    validate(orderValidation.addTrackingNumber),
    OrderControllers.addTrackingNumber
  );
router
  .route("/get-trackingNobyId/:id")
  .put(auth("adminAccess"), OrderControllers.getTrackingNumberbyId);
router
  .route("/update-tracking/:id")
  .put(auth("adminAccess"), OrderControllers.updateTracking);
router.route("/get-statistics").get(OrderControllers.getStatistics);
router
  .route("/get-statistics-country")
  .get(OrderControllers.getStatisticsCountry);
